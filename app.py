...
app.config["TEMPLATES_AUTO_RELOAD"] = True # Permite que los cambios en plantillas se recarguen automáticamente
...
def index():
    ...
    try:
        ...
        cursor.execute("SELECT id_tipo_encuadernacion, nombre_encuadernacion FROM tipos_encuadernacion ORDER BY nombre_encuadernacion")
        ...
        id_tipo_encuadernacion_filter = request.args.get('id_tipo_encuadernacion_filter', '').strip()
        ...
        if main_search_query:
            ...
            where_clauses.append("(titulo_original LIKE %s OR titulo_espanol LIKE %s OR editorial_nombre LIKE %s OR generos LIKE %s OR autores LIKE %s)")
        ...
        if author_ids_filter:
            author_ids = [int(id_str.strip()) for id_str in author_ids_filter.split(',') if id_str.strip().isdigit()]
            if author_ids: 
        ...
        if genre_ids_filter:
            genre_ids = [int(id_str.strip()) for id_str in genre_ids_filter.split(',') if id_str.strip().isdigit()]
            if genre_ids:
        ...
        return render_template('index.html', obras=processed_obras, search_query=main_search_query, 
                               ...
                               q_title=q_title, q_editorial=q_editorial,
...
def get_editoriales():
    ...
    cursor = conn.cursor(dictionary=True) # <<< MODIFICADO: Usar cursor de diccionario para acceder por nombre
    try:
        query = request.args.get('q', '').lower()
        # MODIFICACIÓN: Consultar la nueva tabla 'editoriales'
        cursor.execute(f"SELECT id_editorial, nombre_editorial FROM editoriales WHERE LOWER(nombre_editorial) LIKE %s ORDER BY nombre_editorial LIMIT 10", ('%' + query + '%',))
...
def get_autores():
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute(f"SELECT id_autor, nombre_autor, apellido_autor FROM autores WHERE LOWER(nombre_autor) LIKE %s OR LOWER(apellido_autor) LIKE %s ORDER BY apellido_autor, nombre_autor LIMIT 10", ('%' + query + '%', '%' + query + '%',))
...
def get_generos():
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute(f"SELECT id_genero, nombre_genero FROM generos WHERE LOWER(nombre_genero) LIKE %s ORDER BY nombre_genero LIMIT 10", ('%' + query + '%',))
...
def get_series():
    ...
    cursor = conn.cursor(dictionary=True)
    cursor.execute(f"SELECT id_serie, nombre_serie, descripcion_serie FROM series WHERE LOWER(nombre_serie) LIKE %s ORDER BY nombre_serie LIMIT 10", ('%' + query + '%',))
...
def get_books_for_series_selection():
    ...
    if search_query:
        ...
        where_clauses.append("(LOWER(o.titulo_espanol) LIKE %s OR LOWER(o.titulo_original) LIKE %s OR LOWER(a.nombre_autor) LIKE %s OR LOWER(a.apellido_autor) LIKE %s)")
...
def check_series_name_exists():
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        query = "SELECT COUNT(*) AS count FROM series WHERE LOWER(nombre_serie) = LOWER(%s)"
        params = [name]
        ...
        print(f"DEBUG API CHECK NAME: Name='{name}', ExcludeID={exclude_id}, Query='{query}', Params={params}, Exists={result['count'] > 0}")
...
def new_series():
    conn = get_db_connection()
    if conn is None:
        return jsonify({"message": "Error al conectar con la base de datos.", "category": "danger", "redirect": url_for('index')}), 500
    
    cursor = conn.cursor(dictionary=True) # Cursor para la ruta new_series
    try: 
        if request.method == 'GET':
            return render_template('series_form.html', series={}, all_books=[], series_id=None, current_series_books={})

        if request.method == 'POST':
            print("DEBUG FLASK POST (new_series): Recibiendo POST request.")
            data = request.get_json(silent=True) 
            print(f"DEBUG FLASK POST (new_series): JSON data recibido por request.get_json(): {data}")
            
            series_data_from_payload = data.get('series_to_add')[0] if data and data.get('series_to_add') else {}
            ...
            selected_books_with_order = series_data_from_payload.get('libros_a_asignar', []) 
            
            print(f"DEBUG FLASK POST (new_series): Extrayendo nombre_serie: '{nombre_serie}'")
            # ... (otros prints de depuración) ...

            if not nombre_serie:
                return jsonify({"message": "El nombre de la serie es obligatorio.", "category": "danger"}), 400

            # --- MODIFICACIÓN CLAVE (Problema 3): Validación de unicidad aquí en el backend ---
            cursor.execute("SELECT id_serie FROM series WHERE LOWER(nombre_serie) = LOWER(%s)", (nombre_serie,))
            existing_series = cursor.fetchone()
            if existing_series:
                return jsonify({"message": f"Ya existe una serie con el nombre '{nombre_serie}'. Por favor, elige un nombre diferente o edita la serie existente.", "category": "danger"}), 409 # 409 Conflict
            ...
            try:
                cursor_sp.callproc('manage_bulk_series_assignments', (
                    json.dumps([]), # p_book_ids_json (vacío para nuevas series)
                    json.dumps([{ # p_series_to_add_json (solo la nueva serie)
                ...
                conn.commit()
                return jsonify({"message": "Serie guardada exitosamente.", "redirect": url_for('list_series'), "category": "success"}), 200
            except mysql.connector.Error as err:
                conn.rollback()
                print(f"Error al guardar la serie (DB Error): {err}")
                return jsonify({"message": f"Error al guardar la serie (DB): {str(err)}", "category": "danger"}), 500
            except Exception as e:
                conn.rollback()
                print(f"Error inesperado al guardar la serie: {e}")
                return jsonify({"message": f"Error inesperado al guardar la serie: {str(e)}", "category": "danger"}), 500
            finally:
                if cursor_sp:
...
def edit_series(series_id):
    ...
    print(f"DEBUG FLASK EDIT SERIES (ROUTE HIT): Request for series_id={series_id}, Method={request.method}")

    cursor = conn.cursor(dictionary=True) 
    try:
        if request.method == 'GET':
            # ... (Lógica GET sin cambios) ...
            cursor.execute("SELECT id_serie, nombre_serie, descripcion_serie FROM series WHERE id_serie = %s", (series_id,))
            series_data = cursor.fetchone()
            if not series_data:
                flash("Serie no encontrada.", "warning")
                return redirect(url_for('index'))
            
            cursor.execute("SELECT id_obra, orden_en_serie FROM obra_serie WHERE id_serie = %s", (series_id,))
            current_series_books = cursor.fetchall()
            
            series_books_map = {book['id_obra']: book['orden_en_serie'] for book in current_series_books}

            return render_template('series_form.html', series=series_data, all_books=[], series_id=series_id, current_series_books=series_books_map)

        if request.method == 'POST':
            print("DEBUG FLASK POST (edit_series): Recibiendo POST request.")
            data = request.get_json(silent=True) 
            print(f"DEBUG FLASK POST (edit_series): JSON data recibido por request.get_json(): {data}")
            ...
            print(f"DEBUG FLASK POST (edit_series): Extrayendo nombre_serie: '{nombre_serie}'")
            print(f"DEBUG FLASK POST (edit_series): series_id de la URL: {series_id}") # NUEVO PRINT DE DEPURACIÓN (Problema 2)
            ...
            cursor_sp = conn.cursor() # Asegurarse de tener un cursor aquí para la llamada al SP
            try:
                cursor_sp.callproc('edit_series_and_books', (
                ...
                conn.commit()
                return jsonify({"message": "Serie actualizada exitosamente.", "redirect": url_for('list_series'), "category": "success"}), 200 
            except mysql.connector.Error as err:
                conn.rollback()
                print(f"Error al actualizar la serie (DB Error): {err}")
                return jsonify({"message": f"Error al actualizar la serie (DB): {str(err)}", "category": "danger"}), 500
            except Exception as e:
                conn.rollback()
                print(f"Error inesperado al actualizar la serie: {e}")
                return jsonify({"message": f"Error inesperado al actualizar la serie: {str(e)}", "category": "danger"}), 500
            finally:
                if cursor_sp:
                    cursor_sp.close()
    finally:
    ...
    try:
        ...
        if request.method == 'GET':
            if id_obra: # Modo edición
                ...
                if obra.get('nombre_editorial') is None and obra.get('id_editorial') is not None:
                    print(f"DEBUG FLASK: nombre_editorial es None para id_editorial={obra['id_editorial']}. Intentando consulta directa.")
                    temp_cursor = conn.cursor(dictionary=True) # Usar un cursor temporal
                    try:
                        temp_cursor.execute("SELECT nombre_editorial FROM editoriales WHERE id_editorial = %s", (obra['id_editorial'],))
                        direct_editorial = temp_cursor.fetchone()
                        if direct_editorial:
                            obra['nombre_editorial'] = direct_editorial['nombre_editorial']
                            print(f"DEBUG FLASK: nombre_editorial recuperado directamente: {obra['nombre_editorial']}")
                        else:
                            print(f"DEBUG FLASK: Consulta directa para id_editorial={obra['id_editorial']} tampoco encontró el nombre. Datos inconsistentes.")
                    except Exception as e:
                        print(f"DEBUG FLASK: Error en consulta directa de editorial: {e}")
                    finally:
            ...
            return render_template('form_libro.html', obra=obra, id_obra=id_obra, 
                                    tipos_obra_list=tipos_obra_list, 
                                    tipos_encuadernacion_list=tipos_encuadernacion_list,
                                    editoriales_list=[]) # Mantenemos vacío ya que no se usa como select
            ...
            if not id_tipo_obra:
                return jsonify({"message": "Error: Selecciona un Tipo de Obra.", "category": "danger"}), 400
            ...
            if not nombre_editorial_from_form: 
                return jsonify({"message": "Error: El campo Editorial es obligatorio.", "category": "danger"}), 400
            ...
            if final_id_editorial_for_sp is None:
                cursor.execute("SELECT id_editorial FROM editoriales WHERE LOWER(nombre_editorial) = LOWER(%s)", (nombre_editorial_from_form,))
                existing_editorial = cursor.fetchone()

                if existing_editorial:
                    final_id_editorial_for_sp = existing_editorial['id_editorial']
                else:
                    cursor.execute("INSERT INTO editoriales (nombre_editorial) VALUES (%s)", (nombre_editorial_from_form,))
                    conn.commit()
                    final_id_editorial_for_sp = cursor.lastrowid
                    print(f"DEBUG FLASK: Nueva editorial creada: {nombre_editorial_from_form} con ID: {final_id_editorial_for_sp}")

            if final_id_editorial_for_sp is None: 
                return jsonify({"message": "Error: No se pudo determinar la editorial final.", "category": "danger"}), 500
            ...
            if not id_tipo_encuadernacion:
                return jsonify({"message": "Error: Selecciona un Tipo de Encuadernación.", "category": "danger"}), 400
            id_tipo_encuadernacion = int(id_tipo_encuadernacion)
            
            nombre_encuadernacion = None
            cursor.execute("SELECT nombre_encuadernacion FROM tipos_encuadernacion WHERE id_tipo_encuadernacion = %s", (id_tipo_encuadernacion,))
            ...
            if encuadernacion_result:
                ...
                return jsonify({"message": "Tipo de encuadernación no válido seleccionado.", "category": "danger"}), 400

            autores_json_str = request.form.get('autores_selected_json', '[]')
            generos_json_str = request.form.get('generos_selected_json', '[]')
            imagenes_externas_json_str = request.form.get('imagenes_externas_json', '[]')

            try:
                autores_list = json.loads(autores_json_str) if autores_json_str.strip() else []
                generos_list = json.loads(generos_json_str) if generos_json_str.strip() else []
                imagenes_list_from_form = json.loads(imagenes_externas_json_str) if imagenes_externas_json_str.strip() else []
            except json.JSONDecodeError as e:
                return jsonify({"message": f"Error en el formato JSON enviado (autores, géneros o imágenes externas): {e}", "category": "danger"}), 400

            uploaded_image_urls = [] 
            all_images_for_sp = uploaded_image_urls + imagenes_list_from_form

            autores_json_for_sp = json.dumps([
                {"nombre": autor['nombre_autor'], "apellido": autor['apellido_autor']}
            ...
            cursor.execute("SELECT tipo FROM tipos_obra WHERE id_tipo_obra = %s", (id_tipo_obra,))
            tipo_obra_result = cursor.fetchone()
            if tipo_obra_result:
                tipo_obra_nombre = tipo_obra_result['tipo']
            else:
                return jsonify({"message": "Tipo de obra no válido.", "category": "danger"}), 400
            
            adquirido_estado_valor = adquirido 

            try:
                ev = 2 if id_obra else 1
                flash_msg = "Obra actualizada exitosamente." if id_obra else "Obra registrada exitosamente."

                cursor.callproc('gestionar_obra', (
                    ev,
                    id_obra,
                    titulo_original,
                    titulo_espanol,
                ...
                return jsonify({"message": flash_msg, "redirect": url_for('index'), "category": "success"})
                
            except mysql.connector.Error as err:
                conn.rollback()
                print(f"Error al guardar la obra: {err}")
                return jsonify({"message": f"Error al guardar la obra: {err}", "category": "danger"}), 500
            except Exception as e:
                conn.rollback()
                print(f"Error inesperado al guardar la obra: {e}")
                return jsonify({"message": f"Error inesperado al guardar la obra: {str(e)}", "category": "danger"}), 500
            finally:
...
def eliminar_libro(id_obra):
    conn = get_db_connection()
    if conn is None:
        return jsonify({"message": "Error de conexión a la base de datos.", "category": "danger", "redirect": url_for('index')}), 500
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # *** FIN MODIFICACIÓN ***
            conn.commit()
            return jsonify({"message": "Obra eliminada exitosamente.", "redirect": url_for('index'), "category": "success"})
        except mysql.connector.Error as err:
            conn.rollback()
            return jsonify({"message": f"Error al eliminar la obra: {err}", "category": "danger"}), 500
        finally:
            if cursor:
                cursor.close()
            if conn and conn.is_connected():
    ...
    if obra_a_eliminar:
        return render_template('confirm_delete.html', obra=obra_a_eliminar, id_obra=id_obra)
    else:
...
def list_series():
    ...
    try:
        ...
        base_series_query = "SELECT id_serie, nombre_serie, descripcion_serie FROM series"
        series_where_clauses = []
        series_query_params = []

        if search_query:
            search_pattern = f"%{search_query}%"
            series_where_clauses.append("(LOWER(nombre_serie) LIKE %s OR LOWER(descripcion_serie) LIKE %s)")
        ...
        for series_item in series_list:
            ...
            series_item['books'] = cursor.fetchall()  # Añadir una clave 'books' a cada diccionario de serie

        return render_template('series_list.html', series_list=series_list, search_query=search_query)
    finally:
        if cursor:
            cursor.close()
        if conn and conn.is_connected():
...
def delete_series(series_id):
    conn = get_db_connection()
    if conn is None:
        return jsonify({"message": "Error al conectar con la base de datos.", "category": "danger", "redirect": url_for('list_series')}), 500
    ...
    try:
        if request.method == 'POST':
            ...
            conn.commit()
            return jsonify({"message": "Serie eliminada exitosamente.", "redirect": url_for('list_series'), "category": "success"}), 200

        # Si es GET, simplemente mostrar la página de confirmación
        cursor.execute("SELECT nombre_serie FROM series WHERE id_serie = %s", (series_id,))
        series_to_delete = cursor.fetchone() # Fetchone para obtener el nombre de la serie
        
        if series_to_delete:
            series_name = series_to_delete[0] # Acceder por índice 0 si cursor no es diccionario
            return render_template('confirm_delete_series.html', series_name=series_name, series_id=series_id)
        else:
        ...
        return jsonify({"message": f"Error al eliminar la serie (DB): {str(err)}", "category": "danger"}), 500
    except Exception as e:
        conn.rollback()
        print(f"Error inesperado al eliminar la serie: {e}")
        return jsonify({"message": f"Error inesperado al eliminar la serie: {str(e)}", "category": "danger"}), 500
    finally:
        if cursor:
            cursor.close()
        if conn and conn.is_connected():
...
MASTER_DATA_CONFIG = {
    'autores': {
        'table': 'autores',
        'id_col': 'id_autor',
        'name_cols': ['nombre_autor', 'apellido_autor'], # Columnas por las que se puede buscar por nombre
...
def list_master_data(data_type):
    ...
    cursor = conn.cursor(dictionary=True) # Usamos cursor de diccionario para fácil acceso por nombre
    try:
        search_query = request.args.get('q', '').strip() # Parámetro de búsqueda para la lista
        ...
        if search_query:
            ...
            if name_search_clauses: # Solo añadir si hay cláusulas de búsqueda de nombre
        ...
        return render_template('master_list.html', 
                               ...
                               config=config)              # Pasar la configuración para usar en la plantilla
    finally:
        if cursor:
            cursor.close()
        if conn and conn.is_connected():
...
MASTER_DATA_FORM_FIELDS = {
    'autores': [
        ...
        {'name': 'apellido_autor', 'label': 'Apellido', 'type': 'text', 'required': True},
    ],
    'generos': [
        {'name': 'nombre_genero', 'label': 'Nombre del Género', 'type': 'text', 'required': True},
    ],
    'tipos_obra': [
        {'name': 'tipo', 'label': 'Tipo de Obra', 'type': 'text', 'required': True},
    ],
    'tipos_encuadernacion': [
        {'name': 'nombre_encuadernacion', 'label': 'Nombre de Encuadernación', 'type': 'text', 'required': True},
    ],
    'editoriales': [ # Solo listado, pero si se habilitara edición, estos serían sus campos
        {'name': 'nombre_editorial', 'label': 'Nombre de la Editorial', 'type': 'text', 'required': True},
    ]
}
...
def manage_master_data_logic(data_type, item_id): # Ahora esta es la función que tiene el código
    conn = get_db_connection()
    if conn is None:
        flash("Error al conectar con la base de datos.", "danger")
        return redirect(url_for('admin_dashboard'))
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            if item_id: # Modo edición
                ...
                if not config.get('edit_url_suffix'):
                    flash(f"Edición no permitida para {config['plural_name']}.", "warning")
        ...
        if request.method == 'POST':
            data = request.get_json(silent=True)
            if not data:
                return jsonify({"message": "Datos no recibidos en formato JSON válido.", "category": "danger"}), 400
            ...
            for field in form_fields:
                ...
                if field['required'] and (value is None or str(value).strip() == ''):
                    return jsonify({"message": f"El campo '{field['label']}' es obligatorio.", "category": "danger"}), 400
                values[field['name']] = str(value).strip() if value is not None else None 

            # Construir la consulta SQL (INSERT o UPDATE)
            if item_id: # Modo edición (UPDATE)
                # Asegurarse de que la edición esté permitida
                if not config.get('edit_url_suffix'):
                    return jsonify({"message": f"Edición no permitida para {config['plural_name']}.", "category": "warning"}), 403
                ...
                if not config.get('create_url_suffix'):
                    return jsonify({"message": f"Creación no permitida para {config['plural_name']}.", "category": "warning"}), 403
            ...
            print(f"DEBUG MASTER_DATA_CRUD: Params: {update_params if item_id else insert_params}")

            cursor.execute(query, update_params if item_id else insert_params)
            conn.commit()

            return jsonify({"message": flash_msg, "redirect": url_for('list_master_data', data_type=data_type), "category": "success"}), 200

    except mysql.connector.Error as err:
        conn.rollback()
        print(f"Error DB al gestionar {data_type}: {err}")
        if err.errno == 1062: # Duplicate entry for key (ej. nombre_genero UNIQUE)
            return jsonify({"message": f"Error: Ya existe un(a) {config['singular_name'].lower()} con este nombre.", "category": "danger"}), 400
        return jsonify({"message": f"Error al guardar {config['singular_name'].lower()}: {str(err)}", "category": "danger"}), 500
    except Exception as e:
        conn.rollback()
        print(f"Error inesperado al gestionar {data_type}: {e}")
        return jsonify({"message": f"Error inesperado al guardar {config['singular_name'].lower()}: {str(e)}", "category": "danger"}), 500
    finally:
        if cursor:
            cursor.close()
        if conn and conn.is_connected():
...
def delete_master_data(data_type, item_id):
    ...
    if conn is None:
        ...
        return jsonify({"message": "Error al conectar con la base de datos.", "category": "danger", "redirect": url_for('admin_dashboard')}), 500

    # Validar data_type
    if data_type not in MASTER_DATA_CONFIG:
        flash("Tipo de dato maestro no válido.", "danger")
        # Devolver JSON de error, ya que esta ruta será llamada por AJAX
        return jsonify({"message": "Tipo de dato maestro no válido.", "category": "danger"}), 404 # 404 Not Found
    ...
    if not config.get('delete_url_suffix'):
        return jsonify({"message": f"Eliminación no permitida para {config['plural_name']}.", "category": "warning"}), 403 # 403 Forbidden
    ...
    try:
        ...
        if data_type == 'autores':
            ...
            if data_type == 'generos':
                cursor.execute("SELECT 1 FROM obra_genero WHERE id_genero = %s LIMIT 1", (item_id,))
                is_in_use = cursor.fetchone() is not None
            elif data_type == 'tipos_obra':
                cursor.execute("SELECT 1 FROM obras WHERE id_tipo_obra = %s LIMIT 1", (item_id,))
                is_in_use = cursor.fetchone() is not None
            elif data_type == 'tipos_encuadernacion':
                cursor.execute("SELECT 1 FROM obras WHERE id_tipo_encuadernacion = %s LIMIT 1", (item_id,))
                is_in_use = cursor.fetchone() is not None
            
            if is_in_use:
                return jsonify({"message": f"No se puede eliminar {config['singular_name'].lower()} porque está siendo utilizado(a) por una o más obras.", "category": "danger"}), 409 # 409 Conflict
            ...
            cursor.execute("SELECT 1 FROM obras WHERE id_editorial = %s LIMIT 1", (item_id,))
            is_in_use = cursor.fetchone() is not None

            if is_in_use:
                return jsonify({"message": f"No se puede eliminar la editorial porque está siendo utilizada por una o más obras. Normalice la tabla de obras primero.", "category": "danger"}), 409 # 409 Conflict
        ...
        if err.errno == 1451: # Cannot delete or update a parent row: a foreign key constraint fails
             return jsonify({"message": f"No se puede eliminar {config['singular_name'].lower()} porque tiene registros relacionados. Elimine primero los registros dependientes.", "category": "danger"}), 409
        return jsonify({"message": f"Error al eliminar {config['singular_name'].lower()}: {str(err)}", "category": "danger"}), 500
    except Exception as e:
        conn.rollback()
        print(f"Error inesperado al eliminar {data_type}: {e}")
        return jsonify({"message": f"Error inesperado al eliminar {config['singular_name'].lower()}: {str(e)}", "category": "danger"}), 500
    finally:
        if cursor:
            cursor.close()
        if conn and conn.is_connected():
...
def rate_book():
    conn = get_db_connection()
    if conn is None:
        return jsonify({"message": "Error al conectar con la base de datos.", "category": "danger"}), 500
    ...
    try:
        ...
        if not id_obra:
            return jsonify({"message": "ID de obra es obligatorio para valorar.", "category": "danger"}), 400
        ...
        if puntuacion is None or comentarios is None or fecha_ranking is None:
            ...
            if puntuacion is None:
                 return jsonify({"message": "La puntuación es obligatoria para guardar una valoración.", "category": "danger"}), 400
        ...
        try:
            ...
            if not (1 <= puntuacion <= 10):
                return jsonify({"message": "La puntuación debe ser entre 1 y 10.", "category": "danger"}), 400
        except (ValueError, TypeError):
            return jsonify({"message": "La puntuación debe ser un número válido.", "category": "danger"}), 400
        ...
        VALUES (%s, %s, %s, %s);
        """
        cursor.execute(insert_update_query, (id_obra, puntuacion, comentarios, fecha_ranking))
        conn.commit()

        return jsonify({"message": "Valoración guardada exitosamente.", "category": "success"}), 200

    except mysql.connector.Error as err:
        conn.rollback()
        print(f"Error al guardar valoración (DB Error): {err}")
        if err.errno == 1062:
            return jsonify({"message": "Ya existe una valoración para esta obra.", "category": "danger"}), 409
        return jsonify({"message": f"Error al guardar valoración: {str(err)}", "category": "danger"}), 500
    except Exception as e:
        conn.rollback()
        print(f"Error inesperado al guardar valoración: {e}")
        return jsonify({"message": f"Error inesperado al guardar valoración: {str(e)}", "category": "danger"}), 500
    finally:
        if cursor:
            cursor.close()
        if conn and conn.is_connected():
...
def delete_rating(id_obra):
    ...
    try:
        if not id_obra:
            return jsonify({"message": "ID de obra es obligatorio para eliminar la valoración.", "category": "danger"}), 400

        cursor.execute("DELETE FROM ranking_personal WHERE id_obra = %s", (id_obra,))
        conn.commit()

        if cursor.rowcount > 0: # rowcount > 0 si se eliminó alguna fila
            return jsonify({"message": "Valoración eliminada exitosamente.", "category": "success"}), 200
        else:
            return jsonify({"message": "No se encontró valoración para eliminar.", "category": "warning"}), 404

    except mysql.connector.Error as err:
        conn.rollback()
        print(f"Error al eliminar valoración (DB Error): {err}")
        return jsonify({"message": f"Error al eliminar valoración: {str(err)}", "category": "danger"}), 500
    except Exception as e:
        conn.rollback()
        print(f"Error inesperado al eliminar valoración: {e}")
        return jsonify({"message": f"Error inesperado al eliminar valoración: {str(e)}", "category": "danger"}), 500
    finally:
        if cursor:
            cursor.close()
        if conn and conn.is_connected():
...
def view_series_detail(series_id):
    ...
    if conn is None:
        flash("Error al conectar con la base de datos.", "danger")
        return redirect(url_for('list_series')) # Redirigir a la lista de series si falla la conexión
    ...
    try:
        ...
        for book_dict in books_in_series:
            current_id_obra = None 
            if isinstance(book_dict, dict) and 'id_obra' in book_dict and book_dict['id_obra'] is not None:
                try:
            ...
            if book_dict['id_obra'] is not None:
                ...
                print(f"DEBUG FLASK SERIES DETAIL: Saltando libro con id_obra faltante/inválido en serie {series_id}: {book_dict}")
        

        return render_template('series_detail.html', 
                               series=series_data, 
                               books=processed_books_in_series)
    finally:
        if cursor:
            cursor.close()
        if conn and conn.is_connected():
...
def dashboard():
    ...
    try:
        ...
        stats['libros_adquiridos'] = next((item['count'] for item in adquiridos_data if item['adquirido'] == 1), 0)
        stats['libros_no_adquiridos'] = next((item['count'] for item in adquiridos_data if item['adquirido'] == 0), 0)
        ...
        cursor.execute("SELECT COUNT(DISTINCT id_editorial) AS total_editoriales FROM editoriales")
        ...
        cursor.execute("SELECT COUNT(*) AS libros_leidos FROM ranking_personal WHERE puntuacion IS NOT NULL")
        stats['libros_leidos'] = cursor.fetchone()['libros_leidos']
        stats['libros_no_leidos'] = stats['total_libros'] - stats['libros_leidos'] # Derivado
        ...
        cursor.execute("SELECT AVG(puntuacion) AS promedio_puntuacion FROM ranking_personal WHERE puntuacion IS NOT NULL")
...
def get_books_by_filter():
    ...
    if not filter_type or not filter_value:
        return jsonify({"message": "filter_type y filter_value son obligatorios.", "category": "danger"}), 400
    ...
    try:
        ...
        if filter_type == 'genre':
            ...
            if filter_value in ['0', '1']:
                ...
                return jsonify({"message": "Valor de filtro 'adquirido' no válido.", "category": "danger"}), 400
            ...
            if filter_value == 'leidos':
                ...
                return jsonify({"message": "Valor de filtro 'read_status' no válido.", "category": "danger"}), 400
        elif filter_type == 'author': # Aunque no hay gráfico para top autores, el filtro es útil
            where_clauses.append("autores LIKE %s")
            query_params.append(f"%{filter_value}%")
        elif filter_type == 'ranking': # Aunque no hay gráfico para ranking, el filtro es útil
            ...
            return jsonify({"message": "Tipo de filtro no válido.", "category": "danger"}), 400
        ...
        for book_dict in books:
            ...
            if book_dict['id_obra'] is not None:
                ...
                print(f"DEBUG API BOOKS_BY_FILTER: Saltando libro con id_obra faltante/inválido: {book_dict}")

        return jsonify(processed_books)
    except Exception as e:
        print(f"Error en get_books_by_filter: {e}")
        return jsonify({"message": f"Error interno del servidor al filtrar libros: {str(e)}", "category": "danger"}), 500
    finally:
        if cursor:
            cursor.close()
        if conn and conn.is_connected():
...
def ranking():
    ...
    if conn is None:
        flash("Error al conectar con la base de datos.", "danger")
        return render_template('ranking.html', ranked_books=[], series_list=[], authors_list=[], current_series_id='', current_author_id='')
    ...
    try:
        ...
        base_query = """
            ...
            LEFT JOIN
                imagenes_obra io ON o.id_obra = io.id_obra
        """
        where_clauses = ["rp.puntuacion IS NOT NULL"] # Solo libros que tienen puntuación
        query_params = []
        
        # Añadir JOINs adicionales si hay filtros
        join_clauses = []
        group_by_clauses = [
            "o.id_obra", "o.titulo_espanol", "o.titulo_original", "rp.puntuacion", 
            "rp.comentarios", "rp.fecha_ranking"
        ]
        ...
        if filter_series_id:
            ...
            group_by_clauses.append("os.id_serie") # Incluir en GROUP BY si se une por serie

        if filter_author_id:
            # Aquí ya tenemos JOIN obra_autor oa y autores a
            where_clauses.append("a.id_autor = %s")
            query_params.append(filter_author_id)
            group_by_clauses.append("a.id_autor") # Incluir en GROUP BY si se une por autor
        ...
        full_query += " ORDER BY rp.puntuacion DESC, o.titulo_espanol" # Ordenación por defecto
        ...
        for book_dict in ranked_books:
            ...
            if book_dict['id_obra'] is not None:
                ...
                print(f"DEBUG RANKING: Saltando libro con id_obra faltante/inválido: {book_dict}")
        
        # Obtener listas de series y autores para los dropdowns de filtro
        cursor.execute("SELECT id_serie, nombre_serie FROM series ORDER BY nombre_serie")
        series_list_for_filter = cursor.fetchall()

        cursor.execute("SELECT id_autor, nombre_autor, apellido_autor FROM autores ORDER BY apellido_autor, nombre_autor")
